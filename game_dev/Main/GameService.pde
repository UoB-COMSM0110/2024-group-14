   ...
   public void respawn() {
    if (!gameService.isGameOver()) {
      ...
      for (Platform platform : platforms) {
        if (platform.platColor != color(255, 0, 0)) {
      ...
      if (nonRedPlatforms.size() > 0) {
        ...
        ball.y = randomPlatform.y - 10;
        ball.x = randomPlatform.getX() + randomPlatform.getWidth() / 2;
      }
    }
  }  
}
